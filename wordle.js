...
const isValidKey = (k) => /^[a-z]$|^(Enter|Backspace)$/.test(k); // Checks for a-z, Enter, Backspace
const gameStats = {
    1: 0,
    2: 0,
    3: 0,
...
let streak = 0;
const buildBoard = (numberOfTries = 6) => {
    ...
    const currentGuessStrArray = []; // Keep track of keys entered. Push and pop to account for backspace
    const grayKeys = [];
    const yellowKeys = [];
    const greenKeys = [];
    const currentRow = () => wordRows[currentRowIndex];
    const isRealWord = (attemptedWord) => fullWordList.includes(attemptedWord);
    const isFiveLetterWord = () => currentGuessStrArray.length === 5;
    const updateKeyboard = () => {
        for (const k of grayKeys)
            keyboard.querySelector(`[data-key="${k}"`).className = 'gray';
        for (const k of yellowKeys)
            keyboard.querySelector(`[data-key="${k}"`).className = 'yellow';
        for (const k of greenKeys)
            keyboard.querySelector(`[data-key="${k}"`).className = 'green';
    };
    ...
    document.body.onkeydown = async (e) => await fireLetter(e.key);
    return main;
};
const sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));
const flourish = async (letterBoxes) => {
...
const defeatAnimation = async (letterBoxes) => {
    for (const letterBox of letterBoxes) {
...
const playAgainModal = ({ resultDisplayContainer, textToCopy }) => {
...
const buildResultDisplay = (correctWord, results) => {
